 ...
 * Time: 13:25
 */
class CeshiReportApiController extends Controller
{
...
//            $this->ReturnError(400,"请求不存在");
//        }
        $appNo = $request->getPost('appNo','700006');
        //$sign = $request->getPost('sign','');
        $from = $request->getPost('from','6');
        $phcode = $request->getPost('phcode','868406021276381');
        $simcode = $request->getPost('simcode','89860116891002145765');
        $tjcode = $request->getPost('tjcode','9');
        $sys = $request->getPost('sys','');
        $mac = $request->getPost('mac','');
        $model =trim($request->getPost('model',''));
        $brand = $request->getPost('brand','');
        $com = $request->getPost('com','46001');
        $watch=$request->getPost('watch','[{"appmd5":"B0CE7A729B9576BDB29F7D2BEB7EAEDD","appname":"高德地图","date":"2017-11-06","pkgname":"com.daohangforxiuzhuo","runcount":1,"runlength":1,"runtime":"2017-11-06 16:38:05"}]');
