 ...
 * Explain:zaker新闻
 */
class IncomeZkns extends Income
{
    ...
    {
        //单独用户
        $uList=BindSample::model()->findAll('type= "zkns" and status = 0 and closed=0 and utype=0');
        ...
        //散量分组
        $count=count($uList);
        $sql="SELECT * FROM (SELECT * FROM app_product_list ORDER BY id DESC) as a where type='zkns' and status=1 and agent!=707 group by agent";
    ...
    public function createBySelf($key = '')
    {
        ...
        foreach ($dcList as $k=>$v) {
            ...
            if(!empty($dcList[$k]) && substr($k,0,6)=="999999")
            {
                ...
                // 刨除独立单包用户的激活量
                $uid213 = 1906;
                $md5213 = 'DE64041A4ABF70EA170C62A6E9A85251';
