...
/**
 * Created by liyashan
 * Date: 15-11-20 下午15:36
 * Explain:meituan
 */
class IncomeMeituan extends Income
{
    private $deductComputeList;

    /**
     * @param string $className
     * @return IncomeMeituan
     */
    ...
    {
        //单独用户
        $uList=BindSample::model()->findAll('type= "meituan" and status = 0 and closed=0 and utype=0');
        ...
        //散量分组
        $count=count($uList);
        $sql="SELECT * FROM (SELECT * FROM app_product_list ORDER BY id DESC) as a where type='meituan' and status=1 and agent!=707 group by agent";
