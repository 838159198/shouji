 ...
 * Explain:手机百度
 */
class IncomeYyzx extends Income
{
    ...
    {
        //单独用户
        $uList=BindSample::model()->findAll('type= "yyzx" and status = 0 and closed=0 and utype=0');
        ...
        //散量分组
        $count=count($uList);
        $sql="SELECT * FROM (SELECT * FROM app_product_list ORDER BY id DESC) as a where type='yyzx' and status=1 and agent!=707 group by agent";
    ...
    public function createBySelf($key = '')
    {
        ...
        foreach ($dcList as $k=>$v) {
            ...
            if(!empty($dcList[$k]) && substr($k,0,6)=="999999")
            {
                ...
                // 刨除独立单包用户的激活量
                $uid213 = 213;
                $md5213 = 'F93C1DABC9BEB853C120A701E31EA07D';
