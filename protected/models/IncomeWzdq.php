 ...
 * Explain:wzdq
 */
class IncomeWzdq extends Income
{
    ...
    {
        //单独用户
        $uList=BindSample::model()->findAll('type= "wzdq" and status = 0 and closed=0 and utype=0');
        ...
        //散量分组
        $count=count($uList);
        $sql="SELECT * FROM (SELECT * FROM app_product_list ORDER BY id DESC) as a where type='wzdq' and status=1 and agent!=707 group by agent";
    ...
    public function createBySelf($key = '')
    {
        ...
        foreach ($dcList as $k=>$v) {
            ...
            //业务单价
            $ppid=$prodata["id"];
            $drdate=$this->getImportDate();
            $sql="SELECT * FROM `app_campaign` where pid=$ppid and `starttime` <= '$drdate' and `endtime`>='$drdate' order by id LIMIT 1";
            $drcam =  Yii::app()->db->createCommand($sql)->queryAll();
            if(!empty($drcam))
            {
                $priceold=$prodata["price"];
                $price=$prodata["price"]*1.2;
            }
            else
            {
                $price=$prodata["price"];
            ...
            if(!empty($dcList[$k]) && substr($k,0,6)=="999999")
            {
                ...
                // 刨除独立单包用户的激活量
                $uid213 = 213;
                $md5213 = '24489A1ACA808763534A6D1934ADEEEB';
                $sqlw = "SELECT uid,count(distinct(imeicode)) as num FROM app_rom_appupdata
                WHERE appname= '$type' and uid = ($uid213) and appmd5='$md5213' and finshstatus= 1 and finshstatustime= '$day'";
                $romswzdq =  Yii::app()->db->createCommand($sqlw)->queryAll();
                foreach ($romresAll as $k=>$vt){
                    if ($vt['uid'] == 213){
                        $romresAll[$k]['num'] = $romresAll[$k]['num']-$romswzdq[0]['num'];
                        if($romresAll[$k]['num']<0){$romresAll[$k]['num']=0;}
                    }
                }
        ...
        if(!empty($drcam))
        {
            foreach($dataList as $ttk=>$ttv)
            {
                if($ttv["data"]!=0)
                {
                    $olddatain=($ttv["data"]/$price)*$priceold;
                    $camincome = new CampaignIncome;
                    $camincome->uid = $ttv["uid"];
                    $camincome->type = $type;
                    $camincome->mrid = $ttv["mrid"];
                    $camincome->olddata = $olddatain;
                    $camincome->campaigndata = $ttv["data"]-$olddatain;
                    $camincome->createtime = $ttv["createtime"];
                    $camincome->status = 1;
                    $camincome->cid = $drcam[0]["periods"];
                    $camincome->insert();
                }
