 ...
 * Explain:lbqlds
 */
class IncomeLbqlds extends Income
{
    ...
    {
        //单独用户
        $uList=BindSample::model()->findAll('type= "lbqlds" and status = 0 and closed=0 and utype=0');
        ...
        //散量分组
        $count=count($uList);
        $sql="SELECT * FROM (SELECT * FROM app_product_list ORDER BY id DESC) as a where type='lbqlds' and status=1 and agent!=707 group by agent";
