...
class ProductController extends MsgController
{
    //压缩包文件写入内容
    ...
    public function actionIndex()
    {
        ...
        if(isset($uidt)){
            ...
            if(empty($userapp) || empty($boxdesk))
            {
                ...
                if(empty($userapp))
                {
                    ...
                    if(empty($softpak) || empty($boxdesk) || empty($mark))
                    {
        ...
        if($member->agent==99){
            ...
            $romPackage = RomPackage::model()->findAllByAttributes(array('uid'=>0,'sign'=>1));
        }
