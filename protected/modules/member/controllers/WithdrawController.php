...
class WithdrawController extends MemberController
{
    ...
    public function actionIndex()
    {
        ...
        //统计会员当月收入合计
        $date = date('Y-m');
        $incomeSum = MemberIncome::getSumByMonthSum($date, $this->uid);
        //隐藏收益
        $hideSum=MemberIncome::getHideIncome($this->uid);
        //将合计数计入结款信息表
    ...
    public function actionSave()
    {
        ...
        if($member_data['bank']=="" || $member_data['bank_no']=="" || $member_data['bank_site']=="" || $member_data['holder']==""){
            Common::redirect($this->createUrl('/member/info/bank'), '请先填写财务信息再进行财务提现');
            exit;
        }
        ...
        try {
            ...
            $t->commit();
            if($member_data['weixin_openid']){
                Weixin::handleTemplatec($member_data['weixin_openid'],$member_data['username'],$member_data['bank'],$member_data['bank_no'],$member_data['holder'],$price );
            }
