 ...
 * 后台首页
 */
class DefaultController extends NewdtController
{
	/**
	 * 昨日收入概况
	 */
...
//		$installdata2 = Yii::app()->db->createCommand($sql2)->queryAll();
	...
	public function actionIncome($m = null,$id = null)
	{
			...
			foreach ($val AS $key => $item) {
				if (in_array($key, array_keys($adList))) {
                    ...
                    if($user->member_manage===false){
                        ...
                        $model=SystemLog::model()->find("type=:type and date =:date and target=:target and is_show=1 and status=1",array(":type"=>SystemLog::TYPE_UPLOAD,":date"=>$datetime,":target"=>strtoupper($key)));
                        if($model){
		...
		// print_r($arrFinal['data']->rawData);exit;
   ...
   //      	foreach ($arrFinal['data']->rawData as $key => $value) {
			// 	if(strtotime($value['date'])>strtotime($final)){
			// 		$arrFinal['data']->rawData[$key]['all_income']='未发布';
			// 		$arrFinal['data']->rawData[$key]['activate_income']='未发布';
			// 	}
			// }
   //      }
		/*2017-12-07  业务流id=3321 zlb  end*/

// print_r($arrFinal['data']);exit;
        if(empty($id))
        {
...
// print_r($dataOld);exit;
		foreach ($diffDays as $k=>$v){
			$datetime=SystemLog::getLogDate($v);
            $model=SystemLog::model()->find("type=:type and date =:date  and is_show=1 and status=1",array(":type"=>SystemLog::TYPE_UPLOAD,":date"=>$datetime));
            if($model){
            	$str = '0.00';
				$newData[$k] = array(
					'id' => '0',
					...
					'all_income' => $dataOld['rawData'][$k]['amount'],
					'date' => $v
				);
...
//			}
			...
			foreach ($arr as $k=> $vt){
                ...
                $user = Yii::app()->user;
                if($user->member_manage===false){
                    $datetime=SystemLog::getLogDate($arr['dates']);
                    $model=SystemLog::model()->find("type=:type and date =:date and target=:target and  is_show=1 and status=1 ",array(":type"=>SystemLog::TYPE_UPLOAD,":date"=>$datetime,":target"=>strtoupper($k)));
                    if($model){

                    }else{
                    	 $model=SystemLog::model()->find("type=:type and date =:date and target=:target and  is_show=0 or type=:type and date =:date and target=:target and status=0 ",
                        array(":type"=>SystemLog::TYPE_UPLOAD,":date"=>$datetime,":target"=>strtoupper($k)));
	                    if($model){
	                        $vt=0.00;
	                    }
                    }
