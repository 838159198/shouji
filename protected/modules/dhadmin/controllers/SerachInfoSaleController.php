<?php
/**
 * Explain:SerachInfoSaleController.php
 * @name 录入资料库
 */
class SerachInfoSaleController extends DhadminController
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['SerachInfo']))
		{
            ...
            $username=$_POST['SerachInfo']['username'];
            if(empty($username))
            {
            }
            else
            {
                //判断用户已注册，更改注册状态
                $invitationcode=Common::getInvitationcode($uid);
                if(!empty($invitationcode))
                {
                    $reg_user = Member::model()->find('username=:username and type=3 and invitationcode=:invitationcode',array(':username'=>$username,":invitationcode"=>$invitationcode));
                    if(!empty($reg_user))
                    {
                        $model->reg_status=1;
                    }
			...
			if($model->save())
			{
				echo '<script type="text/javascript">alert("新增成功！");window.location.href="/dhadmin/serachInfoSale/create"; </script>';
			}
	...
	public function actionUpdate($id)
	{
        ...
        if($uid!=$sid)
        {
            ...
            echo '<script charset="utf-8"  type="text/javascript">alert("您只能修改自己录入的信息！");window.close(); </script>';
            exit;
        }
		...
		{
			echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
			echo '<script charset="utf-8"  type="text/javascript">alert("已为有效用户，不能再次编辑！");window.close(); </script>';
			exit;
		}
		...
		if(isset($_POST['SerachInfo']))
		{
			...
			if($model->save())
			{
				...
				$skip = array('name', 'tel', 'userarea', 'qq', 'content', 'com', 'area', 'username', 'search_id', 'manage_id', 'motifytime', 'tixingtime');
				foreach ($attr as $k => $v) {
					if (empty($v)) continue;
					if (in_array($k, $skip));
