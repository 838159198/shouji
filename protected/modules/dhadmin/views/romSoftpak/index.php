...
<script type="text/javascript">
    ...
    $(function() {
        $('#file_upload').uploadify({
            ...
            'uploader' : '<?php echo Yii::app()->createUrl("dhadmin/romSoftpak/moreUpload")?>',
            ...
            'onSelect' : function(file){
                var parten =/^statV+(\d)+\.(\d)+\(+([1-9][0-9]{5})\)\.apk$/;
...
<h1 class="text-center text-primary">统计软件列表</h1>
<?php
...
<div class="row-fluid">
    <div class="app_button" style="margin-top: 30px;">
        <a href="<?php echo $this->createUrl("romSoftpak/create");?>" class="btn btn-success">单个软件上传</a>
    </div>
...
<?php $this->widget('zii.widgets.grid.CGridView', array(
    ...
    'id' => 'admin-grid',
    'dataProvider' => $model->search($x=1),//$x=1 model区别条件
    'pager'=>array(
        ...
        'prevPageLabel'=>'前一页',//定义上一页按钮的显示文字
                            //关于分页器这个array，具体还有很多属性，可参考CLinkPager的API  'filter'=>CHtml::listData(Boxdata::model()->listDataStatus,'key','value'),
    ),
    'emptyText'=>'没有发现用户',
    'filter' => $model,
    'columns' => array(
        ...
        array(
            'name'=>'uid',
            'value'=>'empty($data->uid)? "":$data->uid',
            'htmlOptions'=>array('style'=>'text-align:center;'),
        ),
        array(
            'name'=>'type',
            'value'=>'$data->type',
            'filter'=>CHtml::listData(RomSoftpak::model()->listDataUserType,'key','value'),
            'type'=>"html",
            'htmlOptions'=>array('style'=>'text-align:center;'),
        ),
        'serial_number',
        ...
        array(
            'name'=>'status',
            'value'=>'$data->xstatus',
            'filter'=>CHtml::listData(RomSoftpak::model()->listDataStatus,'key','value'),
            'type'=>"html",
            'htmlOptions'=>array('style'=>'text-align:center;width:100px;'),
        ),
        array(
        ...
        array(
            'header'=>'操作',
            'class' => 'CButtonColumn',
            'updateButtonUrl'=>'Yii::app()->createUrl("dhadmin/romSoftpak/edit",array("id"=>$data->id));',
