...
var lottery = {
	...
	_enable: function () {
        ...
        this.container.find('a').bind('click', this.beforeAuth);
	},

	// 禁用抽奖
	_disable: function () {
		this.container.find('a').unbind('click', this.beforeAuth);
	},
    // 判断权限
    beforeAuth:function(){
        var _this = lottery;

        if (_this.options.beforeAuth) {
            _this.options.beforeAuth.call(_this);
        }
        //_this._stop();
    ...
    _showAwards : function(){
        var lotterywindow = $("#lotteryMsg");
        lotterywindow.attr("class", "songqian-tips-block");
        setInterval(this._closedAwards, 10000);
    },
    //关闭奖品窗口
    _closedAwards:function(){
        $("#lotteryMsg").attr("class", "songqian-tips-none");
    },
